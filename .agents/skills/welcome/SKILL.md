---
name: welcome
description: Greet the user when a new project starts. This mind was created from the "Local Print Bridge" inspiration, so the welcome introduces that inspiration and immediately starts the adaptation conversation.
---

# Welcome the user (inspiration: Local Print Bridge)

This mind was created from an inspiration -- a published snapshot of apps
another mind built:

- Title: Local Print Bridge
- Slug: `local-print-bridge`
- Description: Print from a Minds mind to the user's own Mac printer over the file-only bridge (proof of concept).
- Manifest: `inspiration-local-print-bridge.md` (at the repo root)

Do ALL of the following in your FIRST response, in the same turn, without
waiting to be asked:

1. Open with a short CUSTOM welcome that names **Local Print Bridge** and gives the
   one-line description above. Do NOT use a generic "Welcome to Minds"
   greeting and do NOT offer a generic suggestions list.
2. Immediately read `inspiration-local-print-bridge.md` at the repo root (reading the
